                ...
                for j in range(1,4):
                    if (sensi[i,j] < Prx):
                        ...
                        if j==1:
                            ...
                            self.bw=500
                        at = airtime(self.sf, 1, 4, self.bw) # 4 is (plen)payload length in byte here
                        if at < minairtime:
                            minairtime = at
                            minsf = self.sf
                            minbw = self.bw
...
Ptx = 14
gamma = 2.0      #is the path loss exponent.
d0 = 1000         #d0 is the reference distance, usually 1 km (or 1 mile) for large cell and 1 m to 10 m for microcell
var = 0           # variance ignored for now
Lpld0 = 127.41    #is the path loss at the reference distance d0, calculated using the Friis free-space path loss model. Unit: Decibel (dB)
GL = 0
